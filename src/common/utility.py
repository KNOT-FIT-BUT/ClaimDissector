...
def print_eval_stats(predictions, labels):
    ...
    cm_string = f"    {'   '.join(preds)}\n" + \
                '\n'.join([f"{l} {' '.join([f'{v:1.2f}' for v in row])}" for l, row in zip(labels, cm)])
    logger.info(f"\nConfusion Matrix\n{cm_string}")
...
def get_random_context_size(minimum, n, param_a=1.5, param_b=0.8):
    x = betabinom(n, param_a, param_b).rvs(1).item() + 1  # +1 so it is between 1 and n
...
def multiset_infinite_iterator(dataloader: dict):
    """
    :param iterators: a dictionary of all datasets and their respective iterators
    """
    # returns sample from every iterator as a dict with dataset keys
    # if the iterator is depleted, it is reinitialized

    iterators = dict()
    for dataset in dataloader.keys():
        iterators[dataset] = iter(dataloader[dataset])

    while True:
        sample = dict()
        for dataset in iterators.keys():
            try:
                sample[dataset] = next(iterators[dataset])
            except StopIteration:
                iterators[dataset] = iter(dataloader[dataset])
                sample[dataset] = next(iterators[dataset])
            except BaseException as e:
                logger.error(f"Exception on dataset {dataset}")
                raise e
        yield sample
