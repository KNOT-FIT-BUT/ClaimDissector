### Taken from https://github.com/CambridgeNLIP/verification-real-world-info-needs/blob/main/modelling/experiments/scripts/eval_t1_t2.py
...
def eval_realfc_official(dataset_predictions, gts, official_ids):
    # construct inputs for official script
    classes = ["supported", "refuted", "neutral"]
    veracity_predictions = {official_id: {
        'label': classes[gt[0]],
        'prediction': classes[pred[0]],
    } for official_id, pred, gt in zip(official_ids, dataset_predictions, gts)}
    evidence_prediction = {official_id: {
        'section_label': classes[gt[0]],
        'sentence_labels': ['evidence' if x < 2 else 'no-evidence' for x in gt[1]],
        'sentence_predictions': ['evidence' if p else 'no-evidence' for p in pred[2]],
    } for official_id, pred, gt in zip(official_ids, dataset_predictions, gts)}
    # Report without model correction
    report_normal, report_normal_json = get_evaluation(
        "CDveracity", "CDevidence", veracity_predictions, evidence_prediction
    )
    ...
    report_corrected, report_corrected_json = get_evaluation(
        "CDveracity", "CDevidence", veracity_predictions, corrected_evidence, is_corrected=True
    )
    return report_corrected, report_corrected_json, report_normal, report_normal_json
...
def get_evaluation(veracity_name, evidence_name, veracity, evidence, is_corrected=False):
    ...
    for ncid, section in veracity.keys():
        ...
        # Edit by MF: What is current_veracity['evidence'] for? Is is not used anywhere!
        # Commented out!
        # if not is_corrected:
    ...
    def print_weighted_results(name, weighted_scores, weighted_tp, weighted_fp, weighted_fn):
        ...
        for label in 'supported refuted neutral'.split(' '):
            ...
            if weighted_precision + weighted_recall == 0:
                ...
                output += f'({label}): Precision: {round(weighted_precision, 3)}; Recall: {round(weighted_recall, 3)};  F1: UNDEFINED\n'
                continue
            else:
...
if __name__ == "__main__":
    ...
    pred_dir = "realfc_predictions"
    metrics_dir = "realfc_metrics"
